# Observability Stack: Prometheus + Grafana + Node Exporter + ShoeHub Application

Stack completo de observabilidade com Prometheus (métricas), Grafana (visualização com provisioning automático), Node Exporter (métricas do sistema) e aplicação ShoeHub (ASP.NET Core com instrumentação Prometheus).
...
- Criar diretórios de provisioning do Grafana
- Fazer build da imagem da aplicação ShoeHub
- Iniciar todos os containers com Docker Compose
- **Grafana já inicia com Prometheus conectado e dashboard pré-carregado!**
...
**Não é necessário configurar manualmente!** Basta acessar o Grafana e o dashboard já estará disponível.
...
docketart.sh                        # Script de inicialização automática
├── stop.sh                         # Script de parada
├── prometheus.yml                  # Configuração do Prometheus
├── README.md                       # Este arquivo
├── grafana/                        # Configurações do Grafana (provisioning)
│   └── provisioning/
│       ├── datasources/
│       │   └── prometheus.yml      # Data source do Prometheus
│       └── dashboards/
│           ├── provider.yml        # Provider de dashboards
│           └── shoehub-dashboard.json  # Dashboard de exempl
